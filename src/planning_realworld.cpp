...
void PathPlanning::computeObstacles()
{
    ...
    //     // std::vector<std::vector<cv::Point>> hullContours = {contour};
    //     // cv::drawContours(hull_region, hullContours, 0, cv::Scalar(255), cv::FILLED);
    //     // cv::imshow("Hull Region", hull_region);
    //     // cv::waitKey(0);
    //     // 复制原始轮廓
    //     std::vector<cv::Point> smoothedContour = contour;

    //     // 滑动窗口平滑处理
    //     for (int i = 0; i < contour.size(); i++) {
    //         int start = std::max(0, i - window_size / 2);
    //         int end = std::min((int)contour.size() - 1, i + window_size / 2);
    ...
    //         smoothedContour[i] = avgPoint / count;  // 计算窗口内点的平均值
    //     }

    //     contour = smoothedContour;
    //     // cv::Mat smooth_hull_region = cv::Mat::zeros(obstacle_layer.size(), CV_8UC1);
    //     // std::vector<std::vector<cv::Point>> smooth_hullContours = {contour};
    //     // cv::drawContours(smooth_hull_region, smooth_hullContours, 0, cv::Scalar(255), cv::FILLED);
    //     // cv::imshow("smooth_hull_region", smooth_hull_region);
    //     // cv::waitKey(0);
    // }
...
// #endif
        ...
        if (result < 0) // 不在凸包内的一般障碍
        {
            ...
            cv::imshow("general obstacle", obstacle);
            cv::waitKey(0);
        ...
        else // 
        {
            ...
            LOG(INFO)<<"contour: "<<contour.front();
            LOG(INFO)<<"contour: "<<contour.at(contour.size()/2);
...
//             // 设定椭圆的长轴长度
//             int long_axis = 20; // 根据需要设定
//             // LOG(INFO)<<contour[0]<<" "<<contour[1];
//             // LOG(INFO)<<contour[0].x - contour[1].x<<" "<<contour[0].y - contour[1].y;
//             // 计算椭圆的角度
//             double angle = atan2(contour.at(contour.size()/2).x - contour.front().x, contour.front().y - contour.at(contour.size()/2).y) * 180.0 / CV_PI;
