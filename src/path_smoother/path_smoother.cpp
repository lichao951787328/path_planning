...
PathSmoother::PathSmoother(cv::Mat map_img, std::vector<cv::Point> & cv_path)
{
 ...
 for (auto & cv_point : cv_path)
 {
  original_path_.emplace_back(cv_point.x, cv_point.y, 0);
  cv::circle(map_img_, cv::Point(cv_point.x, cv_point.y), 1, cv::Scalar(0,255,0), -1);
 }
